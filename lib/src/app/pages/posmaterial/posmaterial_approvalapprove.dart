...
class PosMaterialApprovalApprove extends StatefulWidget {
  const PosMaterialApprovalApprove({ Key? key }) : super(key: key);

  @override
  State<PosMaterialApprovalApprove> createState() => _PosMaterialApprovalApproveState();
}

class _PosMaterialApprovalApproveState extends State<PosMaterialApprovalApprove> {
  ...
  getRole() async {
    ...
    setState(() {
      ...
      if (role == 'ADMIN' && divisi == 'SALES') {
        callApi(int.parse(id!), setApprovalStatus(PosStatus.APPROVE), txtSearch.text, limit, offset);
      }

      if (role == 'ADMIN' && divisi == 'MARKETING') {
        callApiManager(true, setApprovalStatus(PosStatus.APPROVE), txtSearch.text, limit, offset);
      }

      if (role == 'ADMIN' && divisi == 'GM') {
        callApiManager(false, setApprovalStatus(PosStatus.APPROVE), txtSearch.text, limit, offset);
      }
    });
  }

  void callApi(int idSalesArea, int status, String search, int limit, int offset) async {
