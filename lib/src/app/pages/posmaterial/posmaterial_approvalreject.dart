...
class PosMaterialApprovalReject extends StatefulWidget {
  const PosMaterialApprovalReject({Key? key}) : super(key: key);

  @override
  State<PosMaterialApprovalReject> createState() =>
      _PosMaterialApprovalRejectState();
}

class _PosMaterialApprovalRejectState extends State<PosMaterialApprovalReject> {
  ...
  getRole() async {
    ...
    setState(() {
      ...
      if (role == 'ADMIN' && divisi == 'SALES') {
        callApi(int.parse(id!), setApprovalStatus(PosStatus.REJECT), txtSearch.text, limit, offset);
      }

      if (role == 'ADMIN' && divisi == 'MARKETING') {
        callApiManager(true, setApprovalStatus(PosStatus.REJECT), txtSearch.text, limit, offset);
      }

      if (role == 'ADMIN' && divisi == 'GM') {
        callApiManager(false, setApprovalStatus(PosStatus.REJECT), txtSearch.text, limit, offset);
      }
    });
  }
