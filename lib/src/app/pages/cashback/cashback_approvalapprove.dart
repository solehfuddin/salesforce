...
class CashbackApprovalApprove extends StatefulWidget {
  Color defaultColor;
  CashbackApprovalApprove({ Key? key, this.defaultColor = MyColors.greenAccent }) : super(key: key);

  @override
  State<CashbackApprovalApprove> createState() => _CashbackApprovalApproveState();
}

class _CashbackApprovalApproveState extends State<CashbackApprovalApprove> {
  ...
  getRole() async {
    ...
    setState(() {
      ...
      if (role == 'ADMIN' && divisi == 'SALES') {
        callApi(int.parse(id!), 1, txtSearch.text, limit, offset);
      }

      if (role == 'ADMIN' && divisi == 'GM') {
        callApiManager(false, 1, txtSearch.text, limit, offset);
      }
    });
  }
