...
class CashbackApprovalReject extends StatefulWidget {
  Color defaultColor;
  CashbackApprovalReject({Key? key, this.defaultColor = MyColors.greenAccent}) : super(key: key);

  @override
  State<CashbackApprovalReject> createState() => _CashbackApprovalRejectState();
}

class _CashbackApprovalRejectState extends State<CashbackApprovalReject> {
  ...
  getRole() async {
    ...
    setState(() {
      ...
      if (role == 'ADMIN' && divisi == 'SALES') {
        callApi(int.parse(id!), 2, txtSearch.text, limit, offset);
      }

      if (role == 'ADMIN' && divisi == 'GM') {
        callApiManager(false, 2, txtSearch.text, limit, offset);
      }
    });
  }
