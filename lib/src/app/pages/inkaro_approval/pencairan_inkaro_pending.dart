...
class PendingPencairanInkaro extends StatefulWidget {
  const PendingPencairanInkaro({Key? key}) : super(key: key);

  @override
  State<PendingPencairanInkaro> createState() => _PendingPencairanInkaroState();
}

class _PendingPencairanInkaroState extends State<PendingPencairanInkaro> {
  String search = '';
  String? id = '';
  String? role = '';
  String? username = '';
  ...
  Future<List<ListPencairanInkaroHeader>> getPendingBySearch(
    ...
    var url = !isAr
        ? role == 'ADMIN'
            ? '$API_URL/inkaro/getPencairanInkaroHeader?approval_sm=PENDING&search=$input&limit=$pageCount&offset=$startAt'
            : '$API_URL/inkaro/getPencairanInkaroHeader?contract_created_by=$id&status_approval=PENDING&search=$input&limit=$pageCount&offset=$startAt'
        : '$API_URL/inkaro/getPencairanInkaroHeader?approval_am=PENDING&search=$input&limit=$pageCount&offset=$startAt';
    tmpList.clear();
  ...
  Future<List<ListPencairanInkaroHeader>> getPendingData(bool isAr) async {
    List<ListPencairanInkaroHeader> list = List.empty(growable: true);
    const timeout = 15;
    var url = !isAr
        ? role == 'ADMIN'
            ? '$API_URL/inkaro/getPencairanInkaroHeader?approval_sm=PENDING&limit=$pageCount&offset=$startAt'
            : '$API_URL/inkaro/getPencairanInkaroHeader?contract_created_by=$id&status_approval=PENDING&limit=$pageCount&offset=$startAt'
        : '$API_URL/inkaro/getPencairanInkaroHeader?approval_am=PENDING&limit=$pageCount&offset=$startAt';
    tmpList.clear();
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      if (constraints.maxWidth > 600 ||
          MediaQuery.of(context).orientation == Orientation.landscape) {
        return childPendingPencairanInkaro(isHorizontal: true);
      }
      return childPendingPencairanInkaro(isHorizontal: false);
    });
  }

  Widget childPendingPencairanInkaro({bool isHorizontal = false}) {
    return Scaffold(
