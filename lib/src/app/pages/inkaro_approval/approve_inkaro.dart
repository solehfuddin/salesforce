...
class ApproveInkaro extends StatefulWidget {
  const ApproveInkaro({Key? key}) : super(key: key);

  @override
  State<ApproveInkaro> createState() => _ApproveInkaroState();
}

class _ApproveInkaroState extends State<ApproveInkaro> {
  String search = '';
  String? id = '';
  String? role = '';
  String? username = '';
  ...
  Future<List<ListInkaroHeader>> getApproveBySearch(String input, bool isAr,
      {bool isHorizontal = false}) async {
    List<ListInkaroHeader> list = List.empty(growable: true);
    const timeout = 15;
    var url = !isAr
        ? '$API_URL/inkaro/getInkaroHeader?approval_sm=APPROVE&search=$input&limit=$pageCount&offset=$startAt'
        : '$API_URL/inkaro/getInkaroHeader?approval_am=APPROVE&search=$input&limit=$pageCount&offset=$startAt';
    tmpList.clear();
  ...
  Future<List<ListInkaroHeader>> getApproveData(bool isAr) async {
    List<ListInkaroHeader> list = List.empty(growable: true);
    const timeout = 15;
    var url = !isAr
        ? '$API_URL/inkaro/getInkaroHeader?approval_sm=APPROVE&limit=$pageCount&offset=$startAt'
        : '$API_URL/inkaro/getInkaroHeader?approval_am=APPROVE&limit=$pageCount&offset=$startAt';
    tmpList.clear();
