...
class Marketingexpense_Approvalpending extends StatefulWidget {
  const Marketingexpense_Approvalpending({Key? key}) : super(key: key);

  @override
  State<Marketingexpense_Approvalpending> createState() =>
      _Marketingexpense_ApprovalpendingState();
}

// ignore: camel_case_types
class _Marketingexpense_ApprovalpendingState
    extends State<Marketingexpense_Approvalpending> {
  ...
  void initState() {
    if (myController.sessionDivisi == "SALES") {
      callApi(int.parse(myController.sessionId), 0, txtSearch.text, limit, offset);
    }

    if (myController.sessionDivisi == "GM") {
      callApiManager(0, txtSearch.text, limit, offset);
    }
  ...
  Future<void> _refreshData() async {
    setState(() {
      if (myController.sessionDivisi == "SALES") {
        callApi(
            int.parse(myController.sessionId), 0, txtSearch.text, limit, offset);
      }
